...
import {
  ...
  updateListing,
  uploadListingImage
} from '@/features/services/listing_crud';
...
const eventTypes = ['Birthday', 'Wedding', 'Corporate', 'Funeral', 'Other'];

// JC: Available serving styles for dropdowns
const servingStyles = ['Buffet', 'Plated', 'Cocktail', 'Family Style'];
...
export default function AddListingModal({ isOpen, onClose }: Props) {
  ...
  function validate() {
    ...
    if (!priceRange || isNaN(Number(priceRange)) || Number(priceRange) < 1)
      newErrors.priceRange = 'Valid price range is required';
    ...
    if (!servingStyle)
      newErrors.servingStyle = 'Serving style is required';

    if (!numStaff || isNaN(Number(numStaff)) || Number(numStaff) < 1)
      newErrors.numStaff = 'Valid number of staff is required';

    if (!numGuests || isNaN(Number(numGuests)) || Number(numGuests) < 1)
      newErrors.numGuests = 'Valid number of guests is required';
  ...
  return (
    <div
      className={cn(
        'fixed inset-0 bg-gray-800/30 backdrop-blur-sm z-50 flex',
    ...
    >
      <div
        className={cn(
          ...
          'relative transform transition-all duration-300 scale-100'
        )}
      >
        {/* Close Button */}
        <button
          ...
          className={cn(
            'cursor-pointer absolute top-4 right-4 p-2 text-teal-600',
            'hover:text-white hover:bg-teal-700 rounded-full',
            'transition-colors z-10 bg-white border border-gray-300 shadow'
          )}
          ...
          <form onSubmit={handleSubmit} className='space-y-5'>
            <div>
              ...
              <input
                ...
                required
                className={cn(
                  `w-full border ${errors.title ? 'border-red-500' : 'border-gray-300'}`,
                  'rounded-md px-3 py-2 focus:outline-none focus:border-teal-500'
                )}
              />
              {errors.title && (
              ...
              <div className='w-1/2'>
                ...
                {eventType === 'Other' && (
                  <input
                    ...
                    onChange={e =>
                      setCustomEventType(sanitizeText(e.target.value))
                    }
                    className={cn(
                      `w-full border ${errors.title ? 'border-red-500' : 'border-gray-300'}`,
                      'rounded-md px-3 py-2 focus:outline-none focus:border-teal-500'
              ...
              <div className='w-1/2 flex gap-4'>
                <div className='w-1/2'>
                  ...
                  <input
                    ...
                    value={numStaff}
                    onChange={e =>
                      setNumStaff(e.target.value.replace(/[^0-9]/g, ''))
                    }
                    required
                    className={cn(
                      `w-full border ${errors.title ? 'border-red-500' : 'border-gray-300'}`,
                ...
                <div className='w-1/2'>
                  ...
                  <input
                    ...
                    value={numGuests}
                    onChange={e =>
                      setNumGuests(e.target.value.replace(/[^0-9]/g, ''))
                    }
                    required
                    className={cn(
                      `w-full border ${errors.title ? 'border-red-500' : 'border-gray-300'}`,
            ...
            <div>
              ...
              {errors.image && (
                <div className='text-sm text-red-600 mt-1'>{errors.image}</div>
              )}
            </div>
            <div>
              <label
            ...
            <div className='flex gap-2 justify-end'>
              ...
              <Button
                ...
                variant='default'
                className={cn(
                  'cursor-pointer bg-teal-50 text-teal-700 border border-gray-300 hover:bg-teal-700',
                  'hover:border-teal-700 hover:text-white transition-all duration-200',
          ...
          <div className='fixed inset-0 bg-black/40 z-50 flex items-center justify-center'>
            <div className='bg-white rounded-xl shadow-xl p-8 max-w-xs w-full flex flex-col items-center'>
              ...
              <div className='flex gap-2 mt-4'>
                ...
                <Button
                  className={cn(
                    'cursor-pointer bg-red-50 text-red-600 border-red-300 hover:bg-red-600 hover:border-red-600',
                    'hover:text-white transition-all duration-200 transform hover:scale-105 shadow-sm hover:shadow-md'
                  )}
                  variant='destructive'
                  onClick={() => {
